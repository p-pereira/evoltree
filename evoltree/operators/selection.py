...
def tournament(population):
    ...
    # Testing!
    # writes the initial population and the final to a text file.
    # only once!
    if params['SAVE_PLOTS']:
        filename1 = path.join(params['FILE_PATH'], 'Selection-initialPop.txt')
        filename2 = path.join(params['FILE_PATH'], 'Selection-finalPop.txt')
        if not os.path.isfile(filename1) and params['VERBOSE']:
