...
class Grammar(object):
    ...
    def read_bnf_file(self, file_name):
        ...
        with open(file_name, "r") as bnf:
            ...
            for rule in finditer(self.ruleregex, content, DOTALL):
                ...
                for p in finditer(
                    self.productionregex, rule.group("production"), MULTILINE
                ):
                    # Iterate over all production choices for this rule.
                    # Split production choices of a rule.

                    if (
                        p.group("production") is None
                        or p.group("production").isspace()
                    ):
                    ...
                    for sub_p in finditer(
                        self.productionpartsregex, p.group("production").strip()
                    ):
                        ...
                        if sub_p.group("subrule"):
                            if terminalparts is not None:
                                ...
                                if terminalparts not in self.terminals:
                                    ...
                                    rule.group("rulename")
                                    not in self.terminals[terminalparts]
                                ):
                            ...
                            tmp_production.append(
                                {"symbol": sub_p.group("subrule"), "type": "NT"}
                            )
                ...
                if not rule.group("rulename") in self.rules:
                    ...
                    raise ValueError(
                        "lhs should be unique", rule.group("rulename")
                    )
        ...
        while graph:
            ...
            for edge in graph:
                ...
                if all(
                    [
                        sy["type"] == "T"
                        or self.non_terminals[sy["symbol"]]["expanded"]
                        for sy in edge[1]
                    ]
                ):
    ...
    def set_arity(self):
        ...
        # a T from any NT.
        self.max_arity = max(
            self.non_terminals[NT]["min_steps"] for NT in self.non_terminals
        )
    ...
    def check_all_permutations(self, depth):
        ...
        if depth < self.min_path:
            ...
            # any we can create
            s = (
                "representation.grammar.Grammar.check_all_permutations\n"
        ...
        if depth in self.permutations.keys():
                        ...
                        if j["type"] == "NT":
                            ...
                            for child in self.rules[j["symbol"]]["choices"]:
                                ...
                                if (
                                    ...
                                    key = [
                                        sym["symbol"] for sym in child["choice"]
                                    ]
            ...
            for sy in start_symbols:
                ...
                if str(key) in depth_per_symbol_trees:
                    pos += (
                        depth_per_symbol_trees[str(key)][depth]
        ...
        for rule in sorted(self.rules.keys()):
            ...
            if concat:
                ...
                for choice in concat:
                    ...
                    symbols = [
                        [sym["symbol"], sym["type"]] for sym in choice["choice"]
                    ]
