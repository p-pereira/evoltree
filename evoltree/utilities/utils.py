...
def create_lamarck_mapper(params):
    ...
    # Delete existing mappers
    _ = [
        shutil.rmtree(path.join(DATA_PATH, "lamarck", x))
        for x in next(walk(path.join(DATA_PATH, "lamarck")))[1]
    ]
    ...
    mapper_base_file = path.join(DATA_PATH, "lamarck", "mapper_base")
    with open(mapper_base_file, "r") as mf:
        content = mf.read()
    # New mapper file
    ## Create file
    init_filename = path.join(mapper_dir, "__init__.py")
    _ = open(init_filename, "w+").close()

    mapper_filename = path.join(mapper_dir, "mapper.py")
    mapper_file = open(mapper_filename, "w+")
    ## Write base code, common to all mappers
    mapper_file.write(content)
    ## Get data headers
    if params["DATASET_TRAIN"] != "":
        data_file = open(path.join("datasets", params["DATASET_TRAIN"]), "r")
        headers = data_file.readline()[:-1]  # ignore last character: '\n'.
        data_file.close()
        headers = headers.replace(
            params["DATASET_DELIMITER"] + params["TARGET"], ""
        )
    ...
    for n, header in enumerate(headers_list):
        funcs += (
            'def p_{0}(t) : "idx : {1}" ; t[0] = [gen_rnd({2}, {3})]\n'.format(
                str(int(f_num) + n + 1), header, str(n), len(headers_list)
            )
        )
        idx += 't_{header} = "{header}"\n'.format(header=header)
    ## Build tokens
    tokens = "\ntokens = ('NUMBER', 'npwhere', 'x', "
    tokens += " ".join(["'{0}',".format(str(elem)) for elem in headers_list])
    tokens += ")\n"
    ...
    mapper_file.write("parser = yacc(debug=False)\n")
    mapper_file.close()

    # Return mapper
    mapper = "evoltree.utilities.lamarck.{0}.mapper".format(
