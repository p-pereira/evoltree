...
def save_first_front_to_file(stats, end=False, name="first"):
    ...
    for i, ind in enumerate(trackers.best_ever):
        ...
        indTest = [
            f.fitness_functions[0](ind, dist="test"),
    ...
    paretoFront.to_csv(
        path.join(params["FILE_PATH"], "pareto.csv"),
        sep=";",
        header=True,
        index=False,
    )
...
def generate_folders_and_files():
    ...
    if params["EXPERIMENT_NAME"]:
        ...
        path_1 = path.join("evoltree", "results")
    ...
    if params["FOLDER_NAME"] is None:
        if not path.isdir(
            path.join(params["FILE_PATH"], str(params["TIME_STAMP"]))
        ):
        ...
        if not path.isdir(
            path.join(params["FILE_PATH"], params["FOLDER_NAME"])
        ):
...
def save_params_to_file():
    ...
    for param in sorted(params.keys()):
        ...
        if param not in [
            ...
            savefile.write(
                str(param) + ": " + "".join(spaces) + str(params[param]) + "\n"
            )
