...
seed(1234)
# save tree rules
def tree_to_code(tree, feature_names, i, transf, nrow, n):
    tree_ = tree.tree_
    feature_name = [
        feature_names[i] if i != _tree.TREE_UNDEFINED else "undefined!"
        for i in tree_.feature
    ]
    f = open((directory + str(i) + "_" + str(n) + ".txt"), "w+")
    ...
    recurse(0, 1)
    f.close()
    f = open((directory + str(i) + "_" + str(n) + ".txt"), "r")
    rules = f.read()
    f.close()
    return rules
...
for i in range(1,8):
    ...
    dtrain = pd.read_csv(('../datasets/Promos/'+MODE+'/Train-' + transf + "-" + str(i)+'.csv'), delimiter=";") #read training data
    dtest = pd.read_csv(('../datasets/Promos/'+MODE+'/Test-' + transf + "-" + str(i)+'.csv'), delimiter=";") #read testing data
    ...
    print("Saving trees...")
    for n, dt in enumerate(rf.estimators_):
        rules = tree_to_code(dt, x_train.columns, i, 
                             transf, len(dtrain.index), n)
        #print(rules)
